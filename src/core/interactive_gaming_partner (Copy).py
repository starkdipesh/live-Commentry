...
Response: [Hinglish Speech]"""
    ...
    async def generate_response(self, user_speech=None, proactive=False):
        ...
        try:
            ...
            print("👁️  Analyzing with vision model...")
            visual_facts = await self._get_visual_description(img_b64)
            print(f"   Visual facts: {visual_facts[:100]}...")
            
            # 3. Get Intelligent Reply (The Mind)
            print("🧠 Generating response with thinking model...")
            ...
            if reply:
                ...
                print("❌ No reply generated from thinking model")
                return None
    ...
    def _log_interaction(self, vision_data, user_input, response):
    ...
    async def run(self):
        ...
        if self.mic_available and self.mic:  # ← Add proper check
            try:
            ...
            print("⚠️ No microphone available. Proceeding in PROACTIVE mode only.")
            stop_listening = lambda x: None
